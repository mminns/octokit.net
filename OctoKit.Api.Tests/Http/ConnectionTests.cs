    ...
    public class ConnectionTests
    {
        ...
        public class TheGetMethod
        {
            ...
            public async Task ParsesApiInfoOnResponse()
            {
                ...
                httpClient.Send(Args.Request, Args.CancellationToken).Returns(Task.FromResult(response));
                var pipeline = Substitute.For<IDataFormatPipeline>();
                pipeline.DeserializeResponse<string>(response).Returns(new ApiResponse<string>(response));
            ...
            public async Task ThrowsApiValidationExceptionFor422Response()
            {
                ...
                var pipeLine = Substitute.For<IDataFormatPipeline>();
                pipeLine.Serializer.Returns(serializer);
                var connection = new Connection(new ProductHeaderValue("OctokitTests"),
                    _exampleUri,
                    Substitute.For<ICredentialStore>(),
                    httpClient,
        ...
        public class ThePatchMethod
        {
            ...
            public async Task RunsConfiguredAppWithAppropriateEnv()
            {
                ...
                var pipeline = Substitute.For<IDataFormatPipeline>();
                pipeline.When(p => p.SerializeRequest(Arg.Any<IRequest>())).Do(args => ((IRequest)args[0]).Body = data);
                pipeline.SerializeRequest(Arg.Any<IRequest>());
                pipeline.DeserializeResponse<string>(response).Returns(new ApiResponse<string>(response, data));
        ...
        public class ThePutMethod
        {
            ...
            public async Task MakesPutRequestWithData()
            {
                ...
                httpClient.Send(Args.Request, Args.CancellationToken).Returns(Task.FromResult(response));
                var pipeline = Substitute.For<IDataFormatPipeline>();
                pipeline.When(p => p.SerializeRequest(Arg.Any<IRequest>())).Do(args => ((IRequest)args[0]).Body = expectedBody);
                pipeline.SerializeRequest(Arg.Any<IRequest>());
                pipeline.DeserializeResponse<string>(response).Returns(new ApiResponse<string>(response, expectedBody));
        ...
        public class ThePostMethod
        {
            ...
            public async Task SendsProperlyFormattedPostRequestWithCorrectHeaders()
            {
                ...
                var pipeline = Substitute.For<IDataFormatPipeline>();
                pipeline.When(p => p.SerializeRequest(Arg.Any<IRequest>())).Do(args =>
                {
                    ((IRequest)args[0]).Body = body;
                    ((IRequest)args[0]).Headers["Accept"] = "application/vnd.github.quicksilver-preview+json; charset=utf-8, application/vnd.github.v3+json; charset=utf-8";
                });
                //pipeline.DeserializeResponse<string>(response).Returns(new ApiResponse<string>(response, body));
        ...
        public class TheConstructor
        {
            ...
            public void EnsuresAbsoluteBaseAddress()
            {
                Assert.Throws<ArgumentException>(() =>
                    new Connection(new ProductHeaderValue("TestRunner"), new Uri("foo", UriKind.Relative), Substitute.For<ICredentialStore>(), Substitute.For<IDataFormatPipeline>()));
                Assert.Throws<ArgumentException>(() =>
                    new Connection(new ProductHeaderValue("TestRunner"), new Uri("foo", UriKind.RelativeOrAbsolute), Substitute.For<ICredentialStore>(), Substitute.For<IDataFormatPipeline>()));
            }
            ...
            public void EnsuresNonNullArguments()
            {
                ...
                // 2 args
                Assert.Throws<ArgumentNullException>(() => new Connection(null, new Uri("https://example.com"), Substitute.For<ICredentialStore>(), Substitute.For<IDataFormatPipeline>())); 
                Assert.Throws<ArgumentNullException>(() => new Connection(new ProductHeaderValue("test"), (Uri)null, Substitute.For<ICredentialStore>(), Substitute.For<IDataFormatPipeline>()));
            ...
            {
                var connection = new Connection(new ProductHeaderValue("OctokitTests"), new Uri("https://github.com/"), Substitute.For<ICredentialStore>(), Substitute.For<IDataFormatPipeline>());
        ...
        public class TheLastAPiInfoProperty
        {
            ...
            public async Task ReturnsObjectIfNotNew()
            {
                ...
                connection.Get<RemoteObject>(new Uri("https://example.com"), TimeSpan.MaxValue);
